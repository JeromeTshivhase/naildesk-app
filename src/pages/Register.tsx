...
import { Phone, Lock, User, Building2, ArrowLeft, FileText, CheckCircle2, Loader2 } from "lucide-react";
import { toast } from "sonner";
import { api } from "../lib/api";
import { useAuth } from "../lib/auth";
import { GlassCard, Button, Toggle } from "../components/ui";

type Step = "form" | "otp" | "terms";
...
export default function RegisterPage() {
  ...
  // Pending auth data after OTP verified — held until T&C accepted
  ...
  async function submitForm(e: React.FormEvent) {
    ...
    try {
      ...
      setMaskedPhone(data.maskedPhone ?? form.phone);
  ...
  async function submitOtp(e: React.FormEvent) {
    ...
    try {
      const { data } = await api.post("/auth/verify-otp", {
        phone: form.phone.replace(/\s/g, ""), otpCode: otp,
      });
      // Store auth data — don't log in yet, show T&C first
      setPendingAuth(data);
      await fetchTerms();
      setStep("terms");    } catch (e: any) {
    ...
    <div style={{
      ...
      <div style={{ width:"100%", maxWidth:400 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:8, marginBottom:20 }}>
          {(["form","otp","terms"] as Step[]).map((s, i) => {
            const done    = ["form","otp","terms"].indexOf(step) > i;
            ...
            return (
              <div key={s} style={{ display:"flex", alignItems:"center", gap:8 }}>
                ...
                {i < 2 && <div style={{ width:24, height:1, background: done ? "var(--primary)" : "var(--border)" }} />}
              </div>
            );
          })}
        </div>
        <div style={{ display:"flex", justifyContent:"space-between", padding:"0 4px", marginBottom:20 }}>
          {["Details","Verify","Terms"].map((l, i) => (
            <span key={l} className="label-mono" style={{ color: ["form","otp","terms"].indexOf(step) === i ? "var(--primary)" : "var(--muted-foreground)", fontSize:9 }}>{l}</span>
          ))}
        </div>
        ...
        {step === "otp" && (
          <GlassCard style={{ padding:28 }}>
            <button onClick={() => setStep("form")}
              style={{ display:"flex", alignItems:"center", gap:6, background:"none", border:"none", cursor:"pointer", color:"var(--muted-foreground)", fontSize:13, marginBottom:20, padding:0 }}>
              <ArrowLeft size={14} /> Back
            </button>
            <h1 className="serif" style={{ fontSize:28, fontWeight:400, marginBottom:4 }}>Check WhatsApp</h1>
            <p style={{ fontSize:13, color:"var(--muted-foreground)", fontWeight:300, marginBottom:24 }}>
            ...
            <form onSubmit={submitOtp} noValidate>
              <input
                ...
                style={{
                  width:"100%", textAlign:"center", fontSize:32, letterSpacing:"0.4em",
                  fontFamily:"'SF Mono',monospace", padding:"16px 12px", marginBottom:16,
                  background:"var(--input)", border:"1px solid var(--border)",
                  borderRadius:"var(--radius)", outline:"none", color:"var(--foreground)",
            ...
            <button
              onClick={async () => {
                try { await api.post("/auth/resend-otp", { phone: form.phone.replace(/\s/g, "") }); toast.success("OTP resent"); }
                catch { toast.error("Could not resend"); }
              }}
              style={{ width:"100%", marginTop:12, padding:10, background:"none", border:"none", cursor:"pointer", fontSize:13, color:"var(--primary)", fontWeight:500 }}
            >
              Resend OTP
            </button>
          </GlassCard>
          ...
          <GlassCard style={{ padding:0, overflow:"hidden" }}>
            ...
            <div style={{ padding:"20px 24px 16px", borderBottom:"1px solid var(--border)" }}>
              <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:6 }}>
                ...
                <div>
                  <p className="label-mono" style={{ color:"var(--primary)" }}>Step 3 of 3</p>
            ...
            {legalLoading ? (
              ...
              <>
                ...
                <div style={{ padding:"16px 24px 24px" }}>
                  ...
                  <button
                    onClick={() => { setStep("otp"); setPendingAuth(null); setAccepted(false); setScrolledToBottom(false); }}
                    style={{ width:"100%", marginTop:8, padding:10, background:"none", border:"none", cursor:"pointer", fontSize:13, color:"var(--muted-foreground)" }}
                  >
                    ← Go back
                  </button>
                </div>
