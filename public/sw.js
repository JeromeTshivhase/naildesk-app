...
self.addEventListener("notificationclick", (event) => {
    ...
    // target is a path like "/appointments/apt-123" — used directly with
    // BrowserRouter (history API). The NAVIGATE message handler in the app
    // must use window.history.pushState / router.navigate, NOT window.location.hash.
            ...
            .then((clients) => {
                ...
                for (const client of clients) {
                    if (client.url.includes(self.location.origin) && "focus" in client) {
                        client.focus();
                        // Send the path — the app router handles it via history.pushState
                        client.postMessage({ type: "NAVIGATE", url: target });
                        return;
                    }
                }
                // Otherwise open a new tab at the full URL
